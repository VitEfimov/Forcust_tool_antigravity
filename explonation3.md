...
**Implementation** (`src/models/hmm.py`):
- **Class**: `RegimeDetector`
- **Method**: `fit(returns)` trains the Gaussian HMM on a **4-year rolling window** (~1008 trading days).
...
- **Class**: `DataLoader`
- **Key Method**: `get_data(symbol)` handles fetching and caching.
- **Source**: Yahoo Finance via `yfinance`
- **Caching**: Local Parquet files stored in `data/cache/` for speed.
- **Symbol Resolution**:
    - “S&P 500” → SPY
    - “NASDAQ 100” → QQQ
...
- **Method**: `prepare_features(df)` extracts predictive signals.
...
### 6.2 Forecasting
### 6.2 Forecasting (Base vs. Derived)
To save computational resources while maintaining accuracy, the system uses a **Tiered Horizon Strategy**:

**Base Models (Deep Training)**:
- **10 Days**: Captures short-term momentum and regime shifts.
- **100 Days**: Captures medium-term business cycle trends.
*These use the full LightGBM + Transformer ensemble.*

**Derived Horizons (Analytical Scaling)**:
- **30d, 60d**: Mathematically derived from the 10d base model.
- **200d, 365d**: Derived from the 100d base model.
*These use volatility scaling laws (Square Root of Time) to project the base forecast forward.*
...
Built using **FastAPI**. Defined in `src/api/routes.py`.

### Endpoints

- **GET /forecast/{symbol}** (`get_forecast`)
    - **Logic**: Calls `_compute_full_forecast` to assemble the ensemble prediction.
    - Returns:
        - Model predictions
        - Simulation bands
        - Current regime
...
- **GET /market/overview** (`get_market_overview`)
    - **Logic**: Calls `_compute_market_overview` (Line 96) to aggregate top 50 assets.
...
### Pages
- **Dashboard** (`Dashboard.jsx`): Main chart + forecast bands.
- **Archive** (`Archive.jsx`): Model performance history.
- **Indices Overview** (`Indices.jsx`): Market-wide perspective.
- **Market Overview** (`MarketOverview.jsx`): Top 50 S&P 500 table.
- **System Status** (`ModelStatus.jsx`): Model architecture status.
...
## 14. Architecture & Connections ("How it all fits")

### 14.1 How Different Training Connects
The system uses a **Walk-Forward** pipeline governed by `src/jobs/daily_run.py` to ensure models are always up-to-date without "peeking" into the future.

1. **Data Ingestion**: The `DataLoader` fetches OHLCV data.
2. **Orchestration**: `daily_run.py` spins up a `WalkForwardForecaster` for each symbol/horizon pair.
3. **Training Loop**:
    - **LightGBM**: Trains on a rolling window (e.g., 730 days).
    - **Regime (HMM)**: Analyzes volatility patterns to output a state (0=Bear, 1=Bull).
    - **Trend (Kalman)**: Fits a smoothing filter to extract "true" price velocity.
    - **Meta-Learning**: A secondary layer (Line 95 of `walk_forward.py`) watches the *errors* of the primary models. If LightGBM starts failing, the Meta-Learner reduces its reliability score.

### 14.2 Final Forecast Aggregation ("The Mathematics")
The final number you see is generated in `src/models/ensemble.py`. It fuses independent signals into one robust prediction.

**Formula**:
```python
Final Prediction = (ML_Signal + Regime_Adj + Trend_Adj + Copula_Adj) * Volatility_Scalar
```

- **ML_Signal**: Weighted average of LightGBM (40%) and Transformers (30%).
- **Regime_Adj**: Boosts forecast in Bull markets, cuts it in Bear markets.
- **Trend_Adj**: Adds momentum if Kalman filter detects strong velocity.
- **Volatility_Scalar**: Dampens the forecast if risk is too high (Higher Volatility = Lower Forecast Confidence).

This aggregation happens **inside the backend** during the Daily Run. The result is saved to the SQLite database.

### 14.3 Which Page Uses All Models?
The **Market Overview** and **Dashboard** pages on the Frontend display these aggregated results.

- **Frontend Component**: `src/frontend/src/components/Dashboard.jsx` (and `MarketOverview.jsx`)
- **API Endpoint**: `GET /forecast/{symbol}` in `src/api/routes.py`
- **What You See**:
    - The **Forecast Price** is the final output of the Ensemble.
    - The **Confidence Score** is the output of the Meta-Learner.
    - The **Regime Label** comes from the HMM.

All models (HMM, LightGBM, Kalman, etc.) contribute to the data users see on the **Dashboard Main View**.
